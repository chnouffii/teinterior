  ...
  return (
    <section className="py-28 lg:py-36">
      ...
      <div className="container-x max-w-2xl text-center">
        <span className="inline-flex h-14 w-14 items-center justify-center rounded-md border border-ink-700 bg-ink-900">
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-2">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.id}
              to={link.path}
              className="inline-flex min-h-[44px] items-center rounded-md border border-ink-700 bg-ink-850 px-4 text-xs font-semibold text-muted transition-colors hover:border-accent/40 hover:text-white"
            >
              {link.label}
            </Link>
          ))}
